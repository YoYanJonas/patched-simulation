...
**Running a simulation is straightforward**: Use the orchestration scripts in the `scripts/` directory. To run a single simulation with the default `rl-full-feature` scenario, simply execute `./scripts/run-scenario.sh rl-full-feature 1`. This script handles everything automatically: it prepares the run directory, starts all Docker containers (allocator service, 3 scheduler services, and iFogSim simulation) in the correct order with health checks, waits for the simulation to complete, stops all containers, and verifies that report files were generated. For multiple runs (e.g., 10 runs for RL policy improvement), use `./scripts/run-scenario.sh rl-full-feature 10` - each run will use the learned models from previous runs while clearing in-memory state between runs.

**Report files and model persistence**: After each run, check the `reports/` directory where you'll find timestamped folders like `reports/2025-11-01T14-30-25_rl-full-feature/` containing simulation reports (both human-readable `.txt` and machine-readable `.json`), config snapshots, and metadata. Learned RL models are automatically saved to the `models/` directory (`models/allocator/` for allocation models, `models/scheduler-node1/`, `scheduler-node2/`, `scheduler-node3/` for scheduling models) and will be loaded in subsequent runs. You can manually copy model files between scenario runs if needed. To use a different scenario, create a new directory under `config/` (e.g., `config/my-scenario/`) with the same structure (`allocator/`, `scheduler/node1/`, `scheduler/node2/`, `scheduler/node3/`, `simulation/`) and reference it: `./scripts/run-scenario.sh my-scenario 5`.
